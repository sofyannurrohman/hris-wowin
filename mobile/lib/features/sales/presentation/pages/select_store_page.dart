...
class SelectStorePage extends StatefulWidget {
  ...
  const SelectStorePage({super.key, this.isQuickOrder = false});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(widget.isQuickOrder ? 'LANGKAH 1 DARI 3' : 'LANGKAH 1 DARI 4', style: GoogleFonts.outfit(fontSize: 10, color: Colors.blueAccent, fontWeight: FontWeight.w800, letterSpacing: 1)),
            Text(widget.isQuickOrder ? 'Pilih Toko Pesanan' : 'Pilih Toko Kunjungan', style: GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w800, color: const Color(0xFF1E293B))),
          ],
        ),
      ),
      body: Column(
        children: [
          // Progress indicator
          LinearProgressIndicator(value: widget.isQuickOrder ? 0.33 : 0.25, minHeight: 4, backgroundColor: Colors.grey.shade200, color: Colors.blueAccent),
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final newStore = await Navigator.push<StoreModel>(
            context,
